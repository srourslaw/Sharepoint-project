...
export const createAdvancedSharePointRoutes = (authService: AuthService, authMiddleware: AuthMiddleware): Router => {
  ...
  // Save edited file back to SharePoint with version management
  router.post('/files/:fileId/update', upload.single('file'), async (req, res) => {
    try {
      ...
      console.log('💾 Saving edited file to SharePoint:', { fileId, hasFile: !!uploadedFile, annotations });
      ...
      if (USE_REAL_SHAREPOINT_API && token) {
        const graphClient = getGraphClient(token);

        try {
          // First, get the original file metadata
          const originalFile = await graphClient.api(`/me/drive/items/${fileId}`).get();
          console.log('📄 Original file:', originalFile.name);

          // Create a backup version comment
          const versionComment = `Edited with annotations on ${new Date().toISOString()}${annotations ? ` (${JSON.parse(annotations).length} annotations)` : ''}`;

          // Upload the new version
          const uploadResponse = await graphClient
            .api(`/me/drive/items/${fileId}/content`)
            .putStream(uploadedFile.buffer);

          console.log('✅ File successfully updated in SharePoint');
          ...
          console.error('❌ Graph API error saving file:', apiError);
          res.status(500).json({
            success: false,
            error: {
              code: 'FILE_SAVE_ERROR',
              message: 'Failed to save file to SharePoint',
              details: apiError.message
            }
          });
        }
        ...
        // Mock response for development
        console.log('🔧 Mock: File save simulated');
        res.json({
          success: true,
          data: {
            fileId,
            fileName: `edited-${uploadedFile.originalname}`,
            versionComment: 'Mock edit - real SharePoint integration disabled',
            lastModified: new Date().toISOString(),
            size: uploadedFile.size,
            annotations: annotations ? JSON.parse(annotations) : [],
            message: 'Mock save successful - real SharePoint integration disabled'
          }
        });
      }
